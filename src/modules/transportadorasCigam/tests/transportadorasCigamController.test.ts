...
describe('TransportadorasCigamController', () => {
  let ctrl: TransportadorasCigamController;
  let svc: any;

  beforeEach(() => {
    svc = { create: vi.fn(), findAll: vi.fn(), findById: vi.fn(), findByIdCigam: vi.fn(), update: vi.fn(), delete: vi.fn() };
    ctrl = new TransportadorasCigamController(svc);
  });
  ...
  it('create returns 201', async () => {
    ...
    const req = mockReq({ body: { id_cigam: 't1', nome: 'Trans' } }) as Request;
    const res = mockRes() as Response;
  ...
  it('findByIdCigam returns 200', async () => {
    ...
    const req = mockReq({ params: { idCigam: 't1' } }) as Request;
    const res = mockRes() as Response;
