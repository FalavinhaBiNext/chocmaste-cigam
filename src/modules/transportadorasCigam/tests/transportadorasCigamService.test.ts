...
describe('TransportadorasCigamService', () => {
  let service: TransportadorasCigamService;
  ...
  beforeEach(() => {
    ...
    service = new TransportadorasCigamService(mockRepository);
  });

  it('should create', async () => {
    mockRepository.create.mockResolvedValue({ id: 'uuid' });
    const result = await service.create({ id_cigam: 't1', nome: 'Trans' } as any);
  ...
  it('should findByIdCigam', async () => {
    mockRepository.findByIdCigam.mockResolvedValue({ id: '123', id_cigam: 't1' });
    expect((await service.findByIdCigam('t1')).id_cigam).toBe('t1');
  });
