...
describe('FormasPagamentoCigamController', () => {
  let ctrl: FormasPagamentoCigamController;
  let svc: any;

  beforeEach(() => {
    svc = { create: vi.fn(), findAll: vi.fn(), findById: vi.fn(), findByIdCigam: vi.fn(), update: vi.fn(), delete: vi.fn() };
    ctrl = new FormasPagamentoCigamController(svc);
  });
  ...
  it('create returns 201', async () => {
    ...
    const req = mockReq({ body: { id_cigam: 'fp1', descricao: 'Cartão' } }) as Request;
    const res = mockRes() as Response;
  ...
  it('findByIdCigam returns 200', async () => {
    ...
    const req = mockReq({ params: { idCigam: 'fp1' } }) as Request;
    const res = mockRes() as Response;
